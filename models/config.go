...
func (e *SysConfig) Create() (SysConfig, error) {
	var doc SysConfig
	i := 0
	orm.DB.Table(e.TableName()).Where("config_name=? or config_key = ?", e.ConfigName, e.ConfigKey).Count(&i)
	if i > 0 {
...
func (e *SysConfig) Update(id int) (update SysConfig, err error) {
	if err = orm.DB.Table(e.TableName()).Where("config_id = ?", id).First(&update).Error; err != nil {
		return
	}
...
func (e *SysConfig) Delete() (success bool, err error) {
	if err = orm.DB.Table(e.TableName()).Where("config_id = ?", e.ConfigId).Delete(&SysConfig{}).Error; err != nil {
		success = false
		return
	}
	success = true
...
func (e *SysConfig) BatchDelete(id []int) (Result bool, err error) {
	if err = orm.DB.Table(e.TableName()).Where("config_id in (?)", id).Delete(&SysConfig{}).Error; err != nil {
		return
	}
	Result = true
	return
