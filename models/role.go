...
func (role *SysRole) GetPage(pageSize int, pageIndex int) ([]SysRole, int, error) {
	...
	table := orm.DB.Select("*").Table("sys_role")
	if role.RoleId != 0 {
...
func (role *SysRole) Insert() (id int, err error) {
	i := 0
	orm.DB.Table(role.TableName()).Where("role_name=? or role_key = ?", role.RoleName, role.RoleKey).Count(&i)
	if i > 0 {
		return 0, errors.New("角色名称或者角色标识已经存在！")
	}
	role.UpdateBy = ""
	result := orm.DB.Table(role.TableName()).Create(&role)
	if result.Error != nil {
		err = result.Error
		return
	}
...
func (role *SysRole) Update(id int) (update SysRole, err error) {
	if err = orm.DB.Table(role.TableName()).First(&update, id).Error; err != nil {
		return
	}
	...
	// 参数1:是要修改的数据
	// 参数2:是修改的数据
	if err = orm.DB.Table(role.TableName()).Model(&update).Updates(&role).Error; err != nil {
		return
	}
	return
}
...
func (role *SysRole) BatchDelete(id []int) (Result bool, err error) {
	if err = orm.DB.Table(role.TableName()).Where("role_id in (?)", id).Delete(SysRole{}).Error; err != nil {
		return
	}
	Result = true
	return
