...
func (e *Post) Delete(id int) (success bool, err error) {
	if err = orm.DB.Table(e.TableName()).Where("post_id = ?", id).Delete(&Post{}).Error; err != nil {
		success = false
		return
	}
	success = true
...
func (e *Post) BatchDelete(id []int) (Result bool, err error) {
	if err = orm.DB.Table(e.TableName()).Where("post_id in (?)", id).Delete(&Post{}).Error; err != nil {
		return
	}
	Result = true
	return
