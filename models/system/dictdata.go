...
func (e *DictData) Create() (DictData, error) {
	...
	i := 0
	orm.DB.Table(e.TableName()).Where("dict_label=? or (dict_label=? and dict_value = ?)", e.DictLabel, e.DictValue).Count(&i)
	if i > 0 {
...
func (e *DictData) Update(id int) (update DictData, err error) {
	if err = orm.DB.Table(e.TableName()).Where("dict_code = ?", id).First(&update).Error; err != nil {
		return
	}
...
func (e *DictData) Delete(id int) (success bool, err error) {
	if err = orm.DB.Table(e.TableName()).Where("dict_code = ?", id).Delete(&DictData{}).Error; err != nil {
		success = false
		return
	}
	success = true
...
func (e *DictData) BatchDelete(id []int) (Result bool, err error) {
	if err = orm.DB.Table(e.TableName()).Where("dict_code in (?)", id).Delete(&DictData{}).Error; err != nil {
		return
	}
	Result = true
	return
